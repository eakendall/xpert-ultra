...
source("Xpert Ultra additional functions plosmed.R")

# create model parameters, in a couple of different formats
params <- readparams(tag=runname, setting=setting, cohortsize=cohortsize, date=date)
estimates <- as.list(params[,"estimate"]); names(estimates)<-rownames(params)

# create patiet cohort, in a couple of different formats
longcohort <- make.longcohort(cohortsize, setting, params)
readcohort <- data.frame(longcohort, stringsAsFactors = F)

# vary parameters across uncertainty distributions
...
# for assay-related parameters with beta distributions
...
# define function that will run model for each draw (#n) of the parameters
...
#run model in parallel for all parameter draws 
